...
/*
-In the given input table, there are rows with missing JOB_ROLE values. 
-Write a query to fill in those blank fields with appropriate values.
...
FROM job_skills)
SELECT row_id,
FIRST_VALUE(job_role) OVER(PARTITION BY Segment ORDER BY row_id),
skills FROM CTE;
