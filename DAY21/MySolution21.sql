...
/* Popular Posts (From Stratascratch):The column 'perc_viewed' in the table 'post_views' 
denotes the percentage of the session 
...
WITH CTE AS 
(SELECT E2.*, session_starttime,session_endtime,
UNIX_TIMESTAMP(session_endtime) - UNIX_TIMESTAMP(session_starttime) AS total_seconds
FROM user_sessions E1
JOIN post_views E2 
ON E1.session_id = E2.session_id)

SELECT post_id,
SUM((perc_viewed/100)*total_seconds) AS 'total_time'
FROM CTE 
GROUP BY post_id
HAVING SUM((perc_viewed/100)*total_seconds)>5
