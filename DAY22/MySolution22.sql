...
WITH ipl_teams AS (SELECT home_team AS 'Teams' FROM ipl_results
UNION 
...
CTE AS 
	...
	FROM ipl_teams t JOIN ipl_results r
	ON r.home_team=t.Teams or r.away_team=t.Teams
...
CTE_FINAL AS (
SELECT *,
COUNT(1) OVER(PARTITION BY Teams,Diff ORDER BY Id RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS 'Streak'
FROM CTE_DIFF)

SELECT r.Teams,COALESCE(MAX(Streak),0) AS 'Highest_Consucetive_Wins'
FROM ipl_teams r
LEFT JOIN CTE_FINAL f 
ON f.Teams=r.Teams
GROUP BY r.Teams
ORDER BY Highest_Consucetive_Wins DESC
