...
SELECT country,age FROM (SELECT *,
ROW_NUMBER() OVER(PARTITION BY country ORDER BY age) AS 'Ranking',
CAST(COUNT(id) OVER(PARTITION BY country ORDER BY age 
RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS DECIMAL) AS 'ppt'
FROM people) AS Base 
WHERE Ranking >= ppt/2 AND Ranking <= (ppt/2)+1
